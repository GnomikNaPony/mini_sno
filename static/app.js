...
function renderState(fill = false) {
  ...
  $('#ai-status').textContent = state.ai_enabled ? `ИИ-редактор: ${state.ai_model}. Ключ настроен; доступ к модели проверяется при генерации. Правила и примеры учитываются при создании текста.` : `Выбран ИИ-редактор ${state.ai_model}. Сейчас работает дайджест из заголовков и аннотаций. Для редакторской переработки по образцам укажите OPENAI_API_KEY в .env на сервере. Цвет обложки работает в обоих режимах.`;
